(()=>{
  ...
  function inject(){
    ...
    $('.lab-head>div>span').textContent='资金去向';$('.lab-head>small').textContent='看每月结余怎样分配，也看安全垫还差多少。';$('#allocationAvailable').previousElementSibling.textContent='扣除安全垫后的余额';$('.allocation-grid>div:first-child>span').textContent='按月支出估算目标金额';
  ...
  function routeMap(route,res,cfg){
    ...
    const reserveTarget=monthlyCost*cfg.reserveMonths,reserveGap=Math.max(0,reserveTarget-savings),reserve=Math.min(budget,Math.round(Math.min(surplus*cfg.reserveShare,reserveGap)));
    ...
    const liquid=Math.max(0,budget-reserve-goal-growth),end=res?.p50?.[st.months-1]||0,available=end-reserveTarget,basis=route.delay>6?'上岗后':'当前';
    return{city:route.city,surplus,reserve,reserveTarget,goal,growth,liquid,available,basis,p10:res?.p10?.[st.months-1]||0,monthlyIncome,monthlyCost,margin};
  }
  ...
  function refresh(){
    ...
    $('#allocationRoute').textContent=`${best.city} · ${cfg.label}（${best.basis}）`;$('#allocationSurplus').textContent=signedMoney(best.surplus);$('#allocationAvailable').textContent=signedMoney(best.available);$('#allocationAvailable').title=`观察期末P50资金减去${cfg.reserveMonths}个月生活支出安全垫`;$('#allocReserve').textContent=money(best.reserveTarget);$('#allocGoal').textContent=money(best.goal);$('#allocGrowth').textContent=money(best.growth);$('#allocStress').textContent=best.p10<0?'P10跌破安全线':'P10仍在安全线之上';
    ...
    $('#bucketBar').innerHTML=bar.map(([k,v])=>`<i class="${k}" style="width:${v/total*100}%" title="${k} ${money(v)}"></i>`).join('');
    refreshCapitalObservatory(best,a,b,cfg);
  }
  const exportButton=$('#exportResult');
  if(exportButton)exportButton.onclick=()=>{const e=st.months-1,a=st.result?.a,b=st.result?.b;if(!a||!b)return;const label=st.goalLabel||'人生目标',model=st.attribution?.model,metric=st.attribution?.[st.attributionMetric==='ending'?'ending':'target'],factors=metric?.contributions?.map(row=>`${row.label}：${signedMoney(row.value)}`).join('\n')||'尚无归因结果',t=`工E千程模拟摘要\n\n期末资金中位数\n${st.a.city}：${money(a.p50[e])}\n${st.b.city}：${money(b.p50[e])}\n\n${label}达成率：${Math.round(a.goal)}% / ${Math.round(b.goal)}%\n差异指标：${metric?.label||'期末资金'}（B − A）${signedMoney(metric?.delta||0)}\n\n因素贡献（Shapley）\n${factors}\n\n模型说明：${model?.paths||1000}条路径 × ${st.months}个月；A/B共用固定随机扰动；${model?.factorCount||5}类因素、${model?.coalitionCount||32}组反事实组合。结果只反映当前演示参数，不是经真实用户数据校准的预测概率，不构成金融建议。`,blob=new Blob([t],{type:'text/plain;charset=utf-8'}),link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download='工E千程-模拟摘要.txt';link.click();setTimeout(()=>URL.revokeObjectURL(link.href),1000);toast('模拟摘要与因素归因已导出')};
